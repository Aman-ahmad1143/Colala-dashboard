...
const RatingAndReviewTable: React.FC<RatingAndReviewTableProps> = ({
  ...
  const renderStars = (rating: number) => {
    return (
      <div className="flex items-center justify-center gap-1">
        ...
        <span className="text-[#E53E3E] text-[14px]">★</span>
      </div>
    );
  };
  ...
  const handleSelectAll = () => {
    ...
    if (onRowSelect) {
      onRowSelect(selectAll ? [] : ratingReviews.map((ratingReview) => ratingReview.id));
    }
  };
          ...
          <tbody>
            {ratingReviews.map((ratingReview, index) => (
              ...
              >
                ...
                <td className="p-4 text-[12px] text-black text-center">
                  {ratingReview.lastRating}
                </td>
                <td className="p-4 text-center">
                  <button
                    onClick={() => handleViewReviews()}
                    className="px-6 py-2 rounded-lg text-[12px] font-medium transition-colors cursor-pointer bg-[#E53E3E] text-white hover:bg-[#D32F2F]"
                  >
                    View Reviews
                  </button>
                </td>
