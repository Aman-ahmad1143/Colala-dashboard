...
const StoreRatingModal: React.FC<StoreRatingModalProps> = ({
  ...
  const renderMainStars = (rating: number) => {
    ...
    for (let i = 0; i < 5; i++) {
      stars.push(
        <span key={i}>
          <svg className={`w-8 h-8 ${i < rating ? 'text-[#E53E3E]' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="h-[600px] -mt-6 overflow-y-auto">
          ...
          <div className="px-6 mt-4">
            ...
            <div className="flex flex-row justify-between border-b border-[#00000080] pb-4 mb-6">
              <span className="text-[#E53E3E] text-[14px]">{rating}.5 Stars</span>
              <span className="text-[#E53E3E] text-[14px]">{totalReviews} Reviews</span>
            </div>

            {/* Reviews */}
            <div className="space-y-5">
            {reviews.map((review) => (
              <div key={review.id} className="border border-[#C3C3C3] rounded-2xl w-full p-3">
                <div className="flex flex-row justify-between mb-3">
                  ...
                  <div>
                    <span className="text-[#00000080] text-sm">{review.date}</span>
                  </div>
                </div>
